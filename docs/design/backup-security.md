...
5. **Network exposure by default.** The server used to listen on every network interface from first launch. New installs now bind to `127.0.0.1` until the admin enables **Network → Allow Access from Other Computers** (existing installs keep LAN access). `npm start` / `npm run dev` are local-only, and `start.bat` asks. Verified: with the local-only bind the LAN address refuses connections while `localhost` works.
6. **Electron runtime advisories.** Electron 36.9.5, which is bundled into the EXE, had many high-severity advisories (including a context-isolation bypass). Upgraded to 44.3.0; `npm audit` reports 0 vulnerabilities, and CI now fails on high or critical advisories.
