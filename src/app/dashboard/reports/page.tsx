...
export default async function ReportsPage() {
  ...
  const employees = await db.all("SELECT id, name, employee_id, department FROM employees ORDER BY name ASC");
  ...
  const leaveRecords = await db.all(`
    SELECT r.id, r.employee_id, e.name, e.employee_id as emp_code, e.department, r.leave_type, r.start_date, r.end_date, r.actual_days, r.reason, r.remarks, r.recorded_at
  ...
  const lateRecords = await db.all(`
    SELECT d.id, d.employee_id, e.name, e.employee_id as emp_code, e.department, d.month_year, d.late_count, d.deducted_cl
