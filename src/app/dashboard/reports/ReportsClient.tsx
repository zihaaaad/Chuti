  ...
  const exportToCSV = () => {
    ...
    if (activeTab === 'logs') {
      ...
      compiledPayroll.forEach(row => {
        csvContent += `"${csvSafe(row.code)}","${csvSafe(row.name)}","${csvSafe(row.department)}",${row.clCount},${row.slCount},${row.elCount},${row.mlCount},${row.lwpCount},${row.totalLeaves},${row.lateCount},${row.deductedCL},${row.netPaidDays}/${row.totalMonthDays}\n`;
      });
    }
